...
exports.getProducts = async (filterString, page = 1, perPage = 24, sortString, priceString) => {
  try {
    const filters = JSON.parse(filterString || '{}');
...
exports.getFilterBar = async (filterString, priceString, category) => {
  ...
  function processRarityCounts(categoryName) {
    ...
    const existingRarity = rarityCounts.find(item => item.name === categoryName);
    if (!existingRarity) {
      if (filters.hasOwnProperty(category) && filters[category].includes(categoryName)) {
