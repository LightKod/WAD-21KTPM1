...
async function UpdateChart() {
  const res = await GetEarningData(14);
...
async function GetTopSetRevenue() {
  try {
    const response = await fetch(`/admin/dashboard/topSetRevenue`);
    const data = await response.json();
...
async function UpdatePieChart() {
  const res = await GetTopSetRevenue();
  console.log(res);
  // Update the chart labels and data
